...
class Product(models.Model):
    ...
    categories = models.ManyToManyField(
        Category, related_name="product_category", verbose_name="Categorias"
    )
    status = models.BooleanField(default=1)
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Criado em")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Editado em")
...
class EntryOutput(models.Model):
    product = models.ForeignKey(
        Product,
        related_name="entryOutput_product",
        on_delete=models.CASCADE,
        verbose_name="Produto",
    )
    client = models.ForeignKey(
        Client,
        related_name="entry_client",
    ...
    unit_price = models.DecimalField(
        max_digits=13, decimal_places=2, verbose_name="Preço unitário"
    )
    status = models.IntegerField(
        default=1,
        choices=((1, "SUCCESS"), (2, "BROKED"), (3, "RETURNED")),
        verbose_name="Estado atual da entrada/saída",
    )
    type = models.IntegerField(
        default=1, choices=((1, "entry"), (2, "output")), verbose_name="Tipo"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Criado em")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Editado em")
...
class HistoryEntryOutput(models.Model):
    entryOutput = models.ForeignKey(
        EntryOutput,
        related_name="entryOutputHistory_entryOutput",
        on_delete=models.CASCADE,
        verbose_name="Entrada e/ou Saída",
    )
    qtd = models.IntegerField(default=0, verbose_name="Quantidade")
